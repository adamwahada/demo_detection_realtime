...
def main():
    ...
    for iteration, (image_st, image_ae), image_penalty in zip(
        ...
        if iteration % 10000 == 0 and iteration > 0:
            ...
            if auc > best_auc:
                print(f"New best AUC! ({best_auc:.4f} -> {auc:.4f}). Saving best models...")
                best_auc = auc
...
def test(test_set, teacher, student, autoencoder, teacher_mean, teacher_std,
    ...
    for image, target, path in tqdm(test_set, desc=desc):
        ...
        print(f"\n[Test Image] Class: {defect_class:<10} | Name: {image_name:<20} | Score: {y_score_image:.4f}")
...
def map_normalization(validation_loader, teacher, student, autoencoder,
    ...
    print(f"\n QUANTILES: st_start={q_st_start}, st_end={q_st_end}, ae_start={q_ae_start}, ae_end={q_ae_end} \n")
